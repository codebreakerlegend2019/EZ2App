...
namespace EasyTwoJuetengApp.Helpers.JdsHelpers
{
    public static class JdsClient
    {
        ...
        public static async Task<JdsResponse> PostAsync<TPram>(string apiBaseAddres, TPram model, string token)
        {
            try
            {
                using (var client = new HttpClient())
        ...
        public static async Task<JdsResponse> PostAsync<TPram>(string apiBaseAddres, TPram model)
        {
            try
            {
                using (var client = new HttpClient())
        ...
        public static async Task<JdsResponse> PutAsync<T>(string apiBaseAddres, T model, string token) where T : new()
        {
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    var json = JsonConvert.SerializeObject(model);
                    var content = new StringContent(json, Encoding.UTF8, "application/json");
                    var response = await client.PutAsync(BaseAddress + apiBaseAddres, content);
                    var requestContent = await response.Content.ReadAsStringAsync();
                    if (response.IsSuccessStatusCode)
                        return new JdsResponse()
                        {
        ...
        public static async Task<JdsResponse> PutAsync<T>(string apiBaseAddres, T model) where T : new()
        {
            try
            {
                using (var client = new HttpClient())
